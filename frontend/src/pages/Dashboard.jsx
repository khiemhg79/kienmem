...
function StatCard({ icon: Icon, label, value, sub, color, alert }) {
  return (
    <div
      className={`bg-white rounded-xl p-5 shadow-sm border ${alert ? "border-red-200 ring-2 ring-red-100" : "border-gray-100"}`}
    >
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div
      className={`px-3 py-2 rounded-lg shadow-lg text-sm border ${val > 29 ? "bg-red-50 border-red-200" : "bg-white border-gray-200"}`}
    >
      <p className="text-gray-500 text-xs">{label}</p>
      <p
        className={`font-bold text-base ${val > 29 ? "text-red-600" : "text-blue-600"}`}
      >
        {val}°C
      </p>
...
export default function Dashboard() {
  ...
  const [selectedRoom, setSelectedRoom] = useState("room301");
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      if (n.status === "fulfilled") setNotifs(n.value.data.slice(0, 10));
      ...
      try {
        const dData = d.status === "fulfilled" ? d.value.data : [];
        const roomDevice = dData.find(dev => dev.room === selectedRoom && dev.type === 'sensor');
        const floor = roomDevice ? roomDevice.floor.toString() : "3";
        const hist = await getSensorHistory(selectedRoom, floor);
  ...
  const handleSaveSimConfig = async () => {
    ...
    try {
      ...
      alert(
        "❌ Lỗi cập nhật cấu hình mô phỏng: " +
  ...
  const tempSensor = sensors.find(
    (s) => s.sensor_type === "temperature" && typeof s.value === "number" && s.room === selectedRoom,
  ) || sensors.find(
    (s) => s.sensor_type === "temperature" && typeof s.value === "number",
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-xs px-2.5 py-1 rounded-full font-medium ${dataSource === "real" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
        >
          {dataSource === "real"
            ? "● Dữ liệu thật (InfluxDB)"
            : "○ Dữ liệu mô phỏng"}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div
          className={`bg-white rounded-xl p-5 shadow-sm border transition-all duration-300 relative ${isHot ? "border-red-200 ring-2 ring-red-100" : "border-gray-100"}`}
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${isHot ? "bg-red-500" : "bg-green-600"}`}
              >
          ...
          {!showSimConfig ? (
            <>
              <p
                className={`text-3xl font-bold ${isHot ? "text-red-600" : "text-gray-900"}`}
              >
                {tempVal ? `${tempVal}°C` : "—"}
              </p>
              <p className="text-xs text-gray-400 mt-1">
                Ngưỡng: 29°C · Max: {maxTemp}°C
              </p>
              {isHot && (
                <p className="text-xs text-red-500 mt-1 font-medium">
                  ⚠ Vượt ngưỡng cảnh báo!
              ...
              {!simConfig.active && (
                <p className="text-xs text-amber-500 mt-1">⏸ Đã tắt mô phỏng</p>
              )}
            </>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="ml-2 border border-gray-200 rounded px-2 py-1 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {Array.from(new Set(devices.filter(d => d.type === 'sensor' && d.room).map(d => d.room))).sort().map(r => (
                  <option key={r} value={r}>{r}</option>
                ))}
                {!devices.find(d => d.room === 'room301' && d.type === 'sensor') && (
          ...
          <p className="text-xs text-gray-400 mt-2">
            {dataSource === "real"
              ? "📡 Dữ liệu thật từ InfluxDB"
              : "⚠ Dữ liệu mô phỏng — publish MQTT để có dữ liệu thật"}
          </p>
        </div>
            ...
            {loading ? (
              ...
              devices.map((d) => (
                ...
                >
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${d.status ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                  >
                    {d.type === "door"
                      ? d.status
                        ? "MỞ"
        ...
        {notifs.length === 0 ? (
          ...
          <div className="space-y-2">
            {notifs.map((n) => (
              <div
                key={n.id}
                className={`flex items-start gap-3 p-3 rounded-lg ${!n.read_at ? "bg-blue-50 border border-blue-100" : "bg-gray-50"}`}
              >
                <Bell
                  className={`w-4 h-4 mt-0.5 flex-shrink-0 ${!n.read_at ? "text-blue-600" : "text-gray-400"}`}
                />
